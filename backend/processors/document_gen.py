...
def generate_pdf(data: dict, user_tier: str = "free") -> str:
    ...
    if images:
        images[0].save(filename, save_all=True, append_images=images[1:], resolution=150)
...
def generate_html(data: dict, user_tier: str = "free") -> str:
    ...
    text_color = "#" + theme.get("text_color", "FFFFFF")
    subtitle_color = "#" + theme.get("subtitle_color", "A0A0B0")
    font_name = theme.get("font_title", "Inter")
    google_font = f"https://fonts.googleapis.com/css2?family={font_name.replace(' ', '+')}:wght@400;600;700;800&display=swap"

    slides_html = ""
    for slide in data.get("slides", []):
        s_title = slide.get("title", "")
        s_content = slide.get("content", "")
        s_type = slide.get("type", "text")

        if s_type == "bullets" and isinstance(s_content, list):
            items_html = "".join([
                f'<div class="bullet-card"><div class="bullet-num">{str(i+1).zfill(2)}</div><div class="bullet-text">{b}</div></div>'
                for i, b in enumerate(s_content)
            ])
            slides_html += f'<div class="section"><div class="section-label">{s_title}</div><div class="bullets-grid">{items_html}</div></div><hr class="divider">'
        else:
            slides_html += f'<div class="section"><div class="section-label">{s_title}</div><p class="text-content">{s_content}</p></div><hr class="divider">'
...
<head>
    ...
    <style>
        * {{ margin:0; padding:0; box-sizing:border-box; }}
        body {{ font-family: '{font_name}', 'Segoe UI', sans-serif; background:{bg}; color:{text_color}; }}
        nav {{ position:fixed; top:0; width:100%; background:{bg}ee; padding:1.2rem 3rem; display:flex; align-items:center; justify-content:space-between; z-index:100; border-bottom: 1px solid {accent}33; backdrop-filter: blur(10px); }}
        nav .logo {{ font-weight:800; font-size:1.1rem; color:{accent}; letter-spacing:1px; }}
        nav a {{ color:{subtitle_color}; text-decoration:none; margin-left:2rem; font-size:0.9rem; }}
        .hero {{ min-height:100vh; display:flex; flex-direction:column; justify-content:center; padding: 8rem 4rem 4rem; max-width:1100px; margin:0 auto; }}
        .hero-tag {{ font-size:12px; letter-spacing:4px; text-transform:uppercase; color:{accent}; margin-bottom:24px; font-weight:600; }}
        .hero h1 {{ font-size:4rem; font-weight:800; line-height:1.1; margin-bottom:1.5rem; max-width:800px; }}
        ...
        .section-label {{ font-size:12px; letter-spacing:4px; text-transform:uppercase; color:{accent}; margin-bottom:40px; font-weight:600; }}
        .bullets-grid {{ display:grid; grid-template-columns:1fr 1fr; gap:16px; }}
        .bullet-card {{ background:{slide_bg}; border:1px solid {accent}22; border-radius:12px; padding:24px 28px; display:flex; gap:20px; align-items:flex-start; }}
        .bullet-num {{ font-size:10px; font-weight:800; color:{accent}; letter-spacing:2px; min-width:28px; padding-top:3px; }}
        .bullet-text {{ font-size:1rem; line-height:1.6; color:{text_color}; }}
        .text-content {{ font-size:1.1rem; line-height:1.9; color:{subtitle_color}; border-left:3px solid {accent}44; padding-left:24px; }}
        ...
        @media(max-width:768px) {{ .hero h1 {{ font-size:2.5rem; }} .bullets-grid {{ grid-template-columns:1fr; }} .section {{ padding:4rem 2rem; }} }}
    </style>
</head>
<body>
    <nav><span class="logo">{data["title"]}</span><div><a href="#contenuto">Contenuto</a><a href="#riepilogo">Riepilogo</a></div></nav>
    <div class="hero">
    ...
    <hr class="divider">
    <div id="contenuto">{slides_html}</div>
